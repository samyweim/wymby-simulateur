...
import { calculerMicro } from "../../src/calculators/micro.js";
...
describe("calculerMicro — BIC Service", () => {
  it("CA normal : abattement 50%, cotisations et net corrects", () => {
    const CA = 50_000;
    const result = calculerMicro(
      {
        CA_HT_RETENU: CA,
        TVA_NETTE_DUE: 0,
    ...
    expect(inter.ABATTEMENT_FORFAITAIRE).toBeCloseTo(abattement_attendu, 0);
    expect(inter.COTISATIONS_SOCIALES_BRUTES).toBeCloseTo(cotisations_attendues, 0);
    expect(inter.BASE_IR_SCENARIO).toBeCloseTo(base_ir, 0);
    expect(result.niveau_fiabilite).toBe("estimation"); // pas d'autres revenus foyer → estimation IR
  });

  it("CA = 0 : cotisations minimales TNS appliquées si micro sans VFL", () => {
    const result = calculerMicro(
      {
        CA_HT_RETENU: 0,
        TVA_NETTE_DUE: 0,
    ...
    expect(inter.CA_HT_RETENU).toBe(0);
    expect(inter.NET_APRES_IR).toBeDefined();
    expect(result.avertissements.length).toBeGreaterThanOrEqual(0);
  });

  it("VFL actif : IR = VFL (taux libératoire), fiabilité non dégradée par IR", () => {
    const CA = 40_000;
    const result = calculerMicro(
      {
        CA_HT_RETENU: CA,
    ...
    expect(inter.IR_ATTRIBUABLE_SCENARIO).toBeCloseTo(vfl_attendu, 0);
  ...
  it("ACRE : cotisations réduites par rapport à sans ACRE", () => {
    const CA = 40_000;
    const sans_acre = calculerMicro(
      {
        CA_HT_RETENU: CA,
        TVA_NETTE_DUE: 0,
        type_micro: "BIC_SERVICE",
    ...
    expect(cotis_avec).toBeLessThan(cotis_sans);
    expect(net_avec).toBeGreaterThan(net_sans);
    expect(avec_acre.intermediaires.REDUCTION_ACRE).toBeGreaterThan(0);
  });
});

describe("calculerMicro — BNC", () => {
  it("Abattement 34% appliqué correctement", () => {
    const CA = 60_000;
    const result = calculerMicro(
      {
        CA_HT_RETENU: CA,
  ...
  it("Abattement minimum (305 €) appliqué si CA très faible", () => {
    ...
    expect(result.intermediaires.ABATTEMENT_FORFAITAIRE).toBe(MIN_ABATTEMENT);
  });

  it("ARCE : SUPER_NET > NET_APRES_IR quand droits_are_restants > 0", () => {
    const CA = 50_000;
    ...
    expect(result.intermediaires.AIDE_ARCE_TRESORERIE).toBeCloseTo(arce_attendu, 0);
    expect(super_net).toBeCloseTo(net_apres_ir + arce_attendu, 0);
    // NET_APRES_IR ≠ SUPER_NET : c'est toute la distinction ARCE
    expect(super_net).not.toBe(net_apres_ir);
  });
});

describe("calculerMicro — BIC Vente", () => {
  it("Abattement 71% appliqué, cotisations 12.3%", () => {
    const CA = 80_000;
    const result = calculerMicro(
      {
        CA_HT_RETENU: CA,
    ...
    expect(result.intermediaires.ABATTEMENT_FORFAITAIRE).toBeCloseTo(abattement_attendu, 0);
    expect(result.intermediaires.COTISATIONS_SOCIALES_BRUTES).toBeCloseTo(cotisations_attendues, 0);
  });

  it("TVA nette déduite du NET_AVANT_IR", () => {
    const CA = 50_000;
    const TVA = 3_500;

    const sans_tva = calculerMicro(
      { CA_HT_RETENU: CA, TVA_NETTE_DUE: 0, type_micro: "BIC_VENTE", option_vfl: false, acre_active: false, nombre_parts_fiscales: 1 },
      P
    );
    const avec_tva = calculerMicro(
      { CA_HT_RETENU: CA, TVA_NETTE_DUE: TVA, type_micro: "BIC_VENTE", option_vfl: false, acre_active: false, nombre_parts_fiscales: 1 },
      P
    );

    const diff = (sans_tva.intermediaires.NET_AVANT_IR ?? 0) - (avec_tva.intermediaires.NET_AVANT_IR ?? 0);
    expect(diff).toBeCloseTo(TVA, 0);
  });

  it("Avec autres_revenus_foyer : fiabilité complet (IR différentiel complet)", () => {
    const CA = 50_000;
    const result = calculerMicro(
      {
        CA_HT_RETENU: CA,
    ...
    expect(result.niveau_fiabilite).toBe("complet");
    expect(result.intermediaires.IR_ATTRIBUABLE_SCENARIO).toBeGreaterThan(0);
  });
});
