...
import { calculerScores } from "../src/comparator.js";

function buildIntermediaires(overrides: Partial<IntermediairesCalcul>): IntermediairesCalcul {
  return {
...
function buildScenario(
  base_id: DetailCalculScenario["base_id"],
...
function buildInput(objectif?: UserInput["OBJECTIF_TRESORERIE"]): UserInput {
  return {
...
describe("calculerScores - objectif tresorerie", () => {
  it("penalise un scenario IS complexe en mode flux_mensuel", () => {
    ...
    const isScores = calculerScores(isScenario, FISCAL_PARAMS_2026, buildInput("flux_mensuel"));
    const eiScores = calculerScores(eiScenario, FISCAL_PARAMS_2026, buildInput("flux_mensuel"));

    expect(isScores.SCORE_GLOBAL_SCENARIO).toBeLessThan(eiScores.SCORE_GLOBAL_SCENARIO);
  });

  it("favorise un scenario IS plus rentable en mode capitalisation", () => {
    const isScenario = buildScenario("G_EURL_IS", {
      NET_APRES_IR: 75_000,
      COUT_TOTAL_SOCIAL_FISCAL: 25_000,
      REDUCTION_ACRE: 2_500,
    ...
    const isScores = calculerScores(isScenario, FISCAL_PARAMS_2026, buildInput("capitalisation"));
    const eiScores = calculerScores(eiScenario, FISCAL_PARAMS_2026, buildInput("capitalisation"));

    expect(isScores.SCORE_GLOBAL_SCENARIO).toBeGreaterThan(eiScores.SCORE_GLOBAL_SCENARIO);
  });

  it("utilise les poids par defaut quand aucun objectif n'est fourni", () => {
    ...
    const scores = calculerScores(scenario, FISCAL_PARAMS_2026, buildInput(undefined));
    const weights = FISCAL_PARAMS_2026.fiscal.CFG_POIDS_SCORE_GLOBAL.par_defaut;
