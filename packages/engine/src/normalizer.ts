...
import type { UserInput, InputsNormalises, OptionTVA } from "@wymby/types";
...
const TAUX_TVA_STANDARD_NOTE =
  "Taux TVA 20 % utilisé par défaut pour conversion TTC→HT en l'absence de données précises. " +
 ...
 */
export function normaliserEntrees(
  input: UserInput,
  params: FP,
  logger: EngineLogger
): NormalisationResult {
  ...
  // ── Normalisation CA ───────────────────────────────────────────────────────
  let CA_HT_RETENU: number;
  let CA_TTC_RETENU: number;
  ...
  if (input.INPUT_MODE_CA === "HT") {
    ...
    if (input.TVA_COLLECTEE_UTILISATEUR !== undefined && input.TVA_COLLECTEE_UTILISATEUR > 0) {
      ...
      CA_TTC_RETENU = caRaw;
      avertissements.push(
        "CA saisi en TTC sans TVA collectée précisée : conversion HT appliquée au taux de 20 % par défaut. " +
  ...
  // ── TVA ────────────────────────────────────────────────────────────────────
  let regime_tva_applicable: OptionTVA = "TVA_FRANCHISE";
  ...
  const TVA_COLLECTEE_THEORIQUE =
    input.TVA_COLLECTEE_UTILISATEUR !== undefined
      ? input.TVA_COLLECTEE_UTILISATEUR
      : 0;

  const TVA_DEDUCTIBLE_RETENUE =
    input.TVA_DEDUCTIBLE_UTILISATEUR !== undefined
      ? input.TVA_DEDUCTIBLE_UTILISATEUR
      : 0;

  const TVA_NETTE_DUE = Math.max(
    0,
    TVA_COLLECTEE_THEORIQUE - TVA_DEDUCTIBLE_RETENUE
  );
  ...
  const RECETTES_PRO_RETENUES =
    CA_HT_RETENU + (input.AUTRES_RECETTES_PRO ?? 0);
  ...
  if (input.CHARGES_DECAISSEES === undefined) {
    avertissements.push(
      "Charges non renseignées : calcul EI réel en mode estimation. " +
  ...
  let NOMBRE_PARTS_FISCALES: number;
  ...
  if (input.NOMBRE_PARTS_FISCALES !== undefined) {
    ...
    // Calcul automatique
    let parts =
      input.SITUATION_FAMILIALE === "marie" || input.SITUATION_FAMILIALE === "pacse"
        ? qfConfig.couple_marie_pacse
        : qfConfig.celibataire;

    const nbEnfants = input.NOMBRE_ENFANTS_A_CHARGE ?? 0;
    if (nbEnfants >= 1) parts += qfConfig.enfant_1_et_2; // +0.5 pour le 1er
    if (nbEnfants >= 2) parts += qfConfig.enfant_1_et_2; // +0.5 pour le 2e
    if (nbEnfants >= 3) parts += (nbEnfants - 2) * qfConfig.enfant_3_et_suivants;

    NOMBRE_PARTS_FISCALES = parts;
    hypotheses.push(
      `Nombre de parts fiscales calculé automatiquement : ${NOMBRE_PARTS_FISCALES} parts`
    );
  }
  ...
  // ── Ancienneté ────────────────────────────────────────────────────────────
  let ANCIENNETE_MOIS: number | null = null;
  if (input.DATE_CREATION_ACTIVITE) {
    const dateCreation = new Date(input.DATE_CREATION_ACTIVITE);
    const today = new Date();
    ANCIENNETE_MOIS = Math.floor(
      (today.getTime() - dateCreation.getTime()) / (1000 * 60 * 60 * 24 * 30.44)
    );
  } else {
    hypotheses.push(
      "Date de création non renseignée : éligibilité ACRE/ARCE calculée sans ancienneté précise"
    );
  }
