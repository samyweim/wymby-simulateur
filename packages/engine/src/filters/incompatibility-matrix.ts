...
export function verifierIncompatibilites(
...
function _verifierScenario(
  ...
  if (
    ...
    motifs.push(
      "Incompatibilité structurelle : VFL (Versement Libératoire) impossible avec un régime réel."
    );
  }

  // ── ZFU nouvelles entrées 2026 fermées ────────────────────────────────────
  if (
    sc.boosters_actifs.includes("BOOST_ZFU_STOCK") &&
    params.zones.CFG_ZFU_NOUVELLES_ENTREES_AUTORISEES === false &&
    input.EST_IMPLANTE_EN_ANCIENNE_ZFU_OUVRANT_DROITS !== true
  ) {
    motifs.push(
      "ZFU fermée aux nouvelles entrées depuis 01/01/2026 (LF 2026 art. 42). " +
  ...
  if (
    sc.boosters_actifs.includes("BOOST_ARCE") &&
  ...
  // ── Non-cumul zones ───────────────────────────────────────────────────────
  const zonesActives = sc.boosters_actifs.filter((b) =>
    ["BOOST_ZFRR", "BOOST_ZFRR_PLUS", "BOOST_QPV", "BOOST_ZFU_STOCK"].includes(b)
  );
  if (zonesActives.length > 1) {
    motifs.push(
      `Non-cumul exonérations de zone : ${zonesActives.join(" + ")} incompatibles simultanément. ` +
  ...
  if (
    (sc.base_id === "G_EURL_IR" || sc.base_id === "G_SASU_IR") &&
    ...
    if (anneesEcoules > dureeMax) {
      motifs.push(
        `Option IR temporaire EURL/SASU expirée (durée max ${dureeMax} exercices, ` +
  ...
  if (
    sc.boosters_actifs.includes("BOOST_ZIP_ZAC") &&
    input.SEGMENT_ACTIVITE !== "sante"
  ) {
    motifs.push(
      "ZIP/ZAC réservé aux professionnels de santé éligibles (hors champ pour ce profil)."
    );
  }

  // ── CPAM aide réservée aux conventionnés ─────────────────────────────────
  if (
    sc.boosters_actifs.includes("BOOST_CPAM") &&
    input.EST_CONVENTIONNE !== true
  ) {
    motifs.push(
      "Aide CPAM réservée aux professionnels de santé conventionnés. Non éligible."
    );
  }

  // ── RAAP réduit hors artistes-auteurs ─────────────────────────────────────
  if (
    sc.boosters_actifs.includes("BOOST_RAAP_REDUIT") &&
    input.SEGMENT_ACTIVITE !== "artiste_auteur"
  ) {
    motifs.push(
      "RAAP réservé aux artistes-auteurs. Non applicable pour ce segment."
    );
  }
...
function _isReelScenario(baseId: string): boolean {
  return (
    baseId === "G_EI_REEL_BIC_IR" ||
