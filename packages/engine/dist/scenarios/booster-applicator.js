...
exports.appliquerBoosters = appliquerBoosters;
/**
 * Applique l'ensemble des boosters actifs sur les valeurs calculées.
 * Chaque booster n'agit que sur son assiette précise.
 */
function appliquerBoosters(boosters, cotisations_brutes, resultat_fiscal, input, params, annee_dans_dispositif, scenario_id) {
    ...
    for (const booster of boosters) {
        switch (booster) {
            case "BOOST_ACRE": {
                ...
                if (r.avertissement)
                    avertissements.push(r.avertissement);
                break;
            }
            case "BOOST_ARCE": {
                ...
                avertissements.push("ARCE modélisée comme flux de trésorerie externe — non incluse dans NET_APRES_IR récurrent. " +
...
// Helpers boosters
// ─────────────────────────────────────────────────────────────────────────────
function _appliquerACRE(cotisations_brutes, input, params) {
    ...
    const taux = dateCreation < dateCharniere
...
function _appliquerQPV(resultat_fiscal, annee_dans_dispositif, input, params) {
    ...
    if (!phase) {
        return {
            exoneration: 0,
            avertissement: `QPV : aucune phase trouvée pour l'année ${annee_dans_dispositif}. ` +
                "Exonération non appliquée.",
        };
    }
    ...
    return {
        exoneration,
        avertissement: partCA < 1
            ? `QPV : exonération proratisée à ${partCA * 100} % du bénéfice (part CA en zone).`
            : undefined,
    };
}
function _appliquerZFUStock(resultat_fiscal, annee_dans_dispositif, params) {
    ...
    if (!phase) {
        return {
            exoneration: 0,
            avertissement: "ZFU stock droits antérieurs : phase non trouvée. " +
...
function _getMontantZIPZAC(params) {
    // ZIP/ZAC — V2 Santé uniquement
    const sante = params.sante;
    if (!sante || !("CFG_PARAM_ZIP_ZAC_MONTANT_FORFAITAIRE" in sante))
        return 0;
    const montant = sante["CFG_PARAM_ZIP_ZAC_MONTANT_FORFAITAIRE"];
    return typeof montant === "number" ? montant : 0;
}
//# sourceMappingURL=booster-applicator.js.map