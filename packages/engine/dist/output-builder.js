...
exports.construireEngineOutput = construireEngineOutput;
function construireEngineOutput(input, norm, qual, scenarios_possibles, scenarios_exclus, calculs, comparaison, recommandation, avertissements_globaux, logger) {
    // ── Inputs normalisés ──────────────────────────────────────────────────────
    const inputs_normalises = {
        profil: {
...
// Helpers privés
// ─────────────────────────────────────────────────────────────────────────────
function _getZoneActive(flags) {
    if (flags.FLAG_ZFRR_PLUS_POSSIBLE)
        return "ZFRR_PLUS";
    if (flags.FLAG_ZFRR_POSSIBLE)
        return "ZFRR";
    if (flags.FLAG_QPV_POSSIBLE)
        return "QPV";
    if (flags.FLAG_ZFU_STOCK_DROITS_POSSIBLE)
        return "ZFU_stock";
    return null;
}
function _calculerNiveauFiabiliteGlobal(calculs) {
    if (calculs.length === 0)
        return "estimation";
    const niveaux = calculs.map((c) => c.niveau_fiabilite);
    if (niveaux.some((n) => n === "estimation"))
        return "estimation";
    if (niveaux.some((n) => n === "partiel"))
        return "partiel";
    return "complet";
}
function _calculerScoreFiabilite(input, norm, nb_scenarios) {
    // Score de 0 à 10
    let score = 10;
    if (input.CHARGES_DECAISSEES === undefined)
        score -= 2;
    if (input.AUTRES_REVENUS_FOYER_IMPOSABLES === undefined)
        score -= 2;
    if (input.RFR_N_2_UTILISATEUR === undefined)
        score -= 1;
    if (input.DATE_CREATION_ACTIVITE === undefined)
        score -= 1;
    if (input.NIVEAU_CERTITUDE_CA === "faible")
        score -= 2;
    if (input.NIVEAU_CERTITUDE_CA === "estimé")
        score -= 1;
    if (nb_scenarios === 0)
        score = 0;
