...
exports.qualifierProfil = qualifierProfil;
/**
 * Qualifie le profil utilisateur et détermine les flags d'éligibilité.
 */
function qualifierProfil(input, norm, params, logger) {
    const elements_a_confirmer = [];
    ...
    // ── Régime TVA ─────────────────────────────────────────────────────────────
    let regime_tva = norm.regime_tva_applicable;
    const seuil_tva = _getSeuilTVAFranchise(input, params, norm.CA_HT_RETENU);
    ...
    if (norm.CA_HT_RETENU > seuil_tva_tolerance) {
        ...
        avertissements.push(`CA (${norm.CA_HT_RETENU} €) dépasse le seuil majoré de tolérance TVA (${seuil_tva_tolerance} €) : ` +
        ...
        avertissements.push(`CA (${norm.CA_HT_RETENU} €) dépasse le seuil de franchise TVA (${seuil_tva} €) : ` +
    ...
    const FLAG_MICRO_BIC_VENTE_POSSIBLE = segment === "generaliste" &&
    ...
    const FLAG_MICRO_BIC_SERVICE_POSSIBLE = segment === "generaliste" &&
    ...
    const FLAG_MICRO_BNC_POSSIBLE = segment === "generaliste" &&
    ...
    const FLAG_DEPASSEMENT_SEUIL_MICRO = !FLAG_MICRO_BIC_VENTE_POSSIBLE &&
    ...
    const FLAG_EI_REEL_BIC_IR_POSSIBLE = segment === "generaliste" &&
    ...
    const FLAG_EI_REEL_BIC_IS_POSSIBLE = FLAG_EI_REEL_BIC_IR_POSSIBLE; // approximation — conditions réelles à affiner
    const FLAG_EI_REEL_BNC_IR_POSSIBLE = segment === "generaliste" &&
        (input.SOUS_SEGMENT_ACTIVITE === "liberal" ||
            input.SOUS_SEGMENT_ACTIVITE === "prestation" ||
            input.SOUS_SEGMENT_ACTIVITE === undefined);
    ...
    const FLAG_EURL_IS_POSSIBLE = segment === "generaliste" &&
        (formeJuridique === "EURL" ||
    ...
    const FLAG_EURL_IR_POSSIBLE = FLAG_EURL_IS_POSSIBLE &&
        _isIRTemporaireValide(input, params);
    const FLAG_SASU_IS_POSSIBLE = segment === "generaliste" &&
        (formeJuridique === "SASU" ||
            formeJuridique === "non_decide" ||
            formeJuridique === undefined);
    const FLAG_SASU_IR_POSSIBLE = FLAG_SASU_IS_POSSIBLE &&
        _isIRTemporaireValide(input, params);
    ...
    const FLAG_RSPM_POSSIBLE = segment === "sante" &&
        input.SOUS_SEGMENT_ACTIVITE === "medecin" &&
        recettesImmobilieres <= (params.social.CFG_TAUX_SOCIAL_RSPM.tranche_2.a ?? 0);
    const FLAG_SANTE_MICRO_POSSIBLE = segment === "sante" && ca <= microParams.CFG_SEUIL_CA_MICRO_BNC;
    const FLAG_SANTE_REEL_POSSIBLE = segment === "sante";
    const FLAG_AIDE_CPAM_POSSIBLE = segment === "sante" &&
        (input.EST_ELIGIBLE_AIDE_CPAM === true ||
            input.SECTEUR_CONVENTIONNEL === "secteur_1" ||
            input.SECTEUR_CONVENTIONNEL === "secteur_2_optam");
    const FLAG_ARTISTE_AUTEUR_BNC_MICRO_POSSIBLE = segment === "artiste_auteur" &&
        (input.MODE_DECLARATION_ARTISTE_AUTEUR === "BNC" ||
            input.MODE_DECLARATION_ARTISTE_AUTEUR === undefined);
    const FLAG_ARTISTE_AUTEUR_TS_POSSIBLE = segment === "artiste_auteur" && input.MODE_DECLARATION_ARTISTE_AUTEUR === "TS";
    const FLAG_RAAP_APPLICABLE = segment === "artiste_auteur" &&
        (input.EST_REDEVABLE_RAAP === true || ca >= params.artiste_auteur.CFG_SEUIL_RAAP);
    const FLAG_LMP_POSSIBLE = segment === "immobilier" &&
        recettesImmobilieres >= params.immobilier.CFG_SEUIL_LMP_RECETTES &&
        recettesImmobilieres > autresRevenusActivite;
    const FLAG_LMNP_MICRO_POSSIBLE = segment === "immobilier" &&
    ...
    if (FLAG_EURL_IR_POSSIBLE || FLAG_SASU_IR_POSSIBLE) {
        elements_a_confirmer.push("Régime IR temporaire (EURL/SASU) : durée limitée à 5 exercices. " +
            "Vérifier la date de début de l'option.");
    }
    // ── VFL ────────────────────────────────────────────────────────────────────
    const { FLAG_VFL_POSSIBLE, FLAG_VFL_INTERDIT } = _qualifierVFL(input, params, avertissements, elements_a_confirmer);
    // ── TVA et dépassement ────────────────────────────────────────────────────
    ...
    if (FLAG_ARCE_POSSIBLE) {
        elements_a_confirmer.push("Éligibilité ARCE déclarée — à confirmer avec Pôle Emploi (France Travail). " +
    ...
    const FLAG_QPV_POSSIBLE = input.EST_IMPLANTE_EN_QPV === true && params.zones.CFG_QPV_ACTIVE === true;
    const FLAG_ZFU_STOCK_DROITS_POSSIBLE = input.EST_IMPLANTE_EN_ANCIENNE_ZFU_OUVRANT_DROITS === true &&
    ...
    if (FLAG_ZFRR_POSSIBLE || FLAG_QPV_POSSIBLE || FLAG_ZFU_STOCK_DROITS_POSSIBLE) {
        elements_a_confirmer.push("Exonération de zone détectée. Vérifier le code postal et la liste officielle des communes éligibles.");
    }
    if ([FLAG_ZFRR_POSSIBLE, FLAG_QPV_POSSIBLE, FLAG_ZFU_STOCK_DROITS_POSSIBLE].filter(Boolean).length > 1) {
        avertissements.push("Plusieurs régimes de zone détectés : " +
    ...
    if (FLAG_TAXE_PUMA_APPLICABLE) {
        avertissements.push("Taxe PUMa (cotisation subsidiaire maladie) potentiellement applicable. " +
    ...
    const FLAG_DONNEES_A_COMPLETER = input.CHARGES_DECAISSEES === undefined ||
...
function _normalizeSegment(segment) {
    ...
    if (normalise === "sante")
        return "sante";
    if (normalise === "artiste_auteur")
        return "artiste_auteur";
    if (normalise === "immobilier")
        return "immobilier";
    return "generaliste";
}
// ─────────────────────────────────────────────────────────────────────────────
// Helpers privés
// ─────────────────────────────────────────────────────────────────────────────
function _getSeuilTVAFranchise(input, params, _ca) {
    const ss = input.SOUS_SEGMENT_ACTIVITE;
    if (ss === "achat_revente")
        return params.tva.CFG_SEUIL_TVA_FRANCHISE_BIC_VENTE;
    if (ss === "liberal")
        return params.tva.CFG_SEUIL_TVA_FRANCHISE_BNC;
    return params.tva.CFG_SEUIL_TVA_FRANCHISE_BIC_SERVICE;
}
function _getSeuilTVATolerance(input, params, _ca) {
    const ss = input.SOUS_SEGMENT_ACTIVITE;
    if (ss === "achat_revente")
        return params.tva.CFG_SEUIL_TVA_TOLERANCE_BIC_VENTE;
    if (ss === "liberal")
        return params.tva.CFG_SEUIL_TVA_TOLERANCE_BNC;
    return params.tva.CFG_SEUIL_TVA_TOLERANCE_BIC_SERVICE;
}
function _qualifierVFL(input, params, avertissements, elements_a_confirmer) {
    ...
    if (rfr === undefined) {
        ...
        elements_a_confirmer.push("RFR N-2 non renseigné : éligibilité au Versement Libératoire calculée en mode estimation. " +
            "Fournir le RFR pour un calcul précis.");
        return { FLAG_VFL_POSSIBLE: true, FLAG_VFL_INTERDIT: false }; // optimiste avec alerte
    }
    if (rfr > seuilRFR) {
        avertissements.push(`RFR N-2 (${rfr} €) dépasse le seuil VFL (${seuilRFR} € pour ${nbParts} parts) : ` +
...
function _isIRTemporaireValide(input, params) {
    const dureeMax = params.temporalite.CFG_DUREE_OPTION_IR_TEMPORAIRE_SOCIETE.nb_exercices_max;
    if (!input.DATE_CREATION_ACTIVITE)
        return true; // on laisse passer, alerte émise par qualifier
    const dateCreation = new Date(input.DATE_CREATION_ACTIVITE);
    const now = new Date();
    const anneesDiff = (now.getTime() - dateCreation.getTime()) / (1000 * 60 * 60 * 24 * 365.25);
    return anneesDiff <= dureeMax;
}
function _isAcrePossible(input, params, avertissements) {
    if (!params.aides.CFG_ACRE_ACTIVE)
        return false;
    if (input.ACRE_DEMANDEE === false)
        return false;
    // ACRE réservée aux créateurs/repreneurs dans les 24 mois de création
    if (input.EST_CREATEUR_REPRENEUR === false)
        return false;
    // Vérification ancienneté si date connue
    if (input.DATE_CREATION_ACTIVITE) {
        const dateCreation = new Date(input.DATE_CREATION_ACTIVITE);
        const moisDepuisCreation = (new Date().getTime() - dateCreation.getTime()) /
            (1000 * 60 * 60 * 24 * 30.44);
        const dureeMois = params.aides.CFG_DUREE_ACRE.hors_micro.duree_mois;
        if (moisDepuisCreation > dureeMois) {
            avertissements.push(`ACRE : durée maximale (${params.aides.CFG_DUREE_ACRE.hors_micro.duree_mois} mois) potentiellement dépassée. ` +
...
function _isArcePossible(input, params) {
    ...
    if (!arceConfig)
        return false;
    return (input.ARCE_DEMANDEE === true &&
        input.EST_BENEFICIAIRE_ARE === true &&
        (input.DROITS_ARE_RESTANTS ?? 0) > 0);
}
function _isPumaPotentielle(input, params) {
