...
exports.determinerScenarioReference = determinerScenarioReference;
exports.calculerEcarts = calculerEcarts;
exports.calculerScores = calculerScores;
exports.construireComparaison = construireComparaison;
exports.determinerRecommandation = determinerRecommandation;
/**
 ...
 */
function determinerScenarioReference(calculs) {
    if (calculs.length === 0)
        return null;
    // Préférer un scénario micro simple (sans booster) comme référence
    const micro = calculs.find((c) => (c.base_id === "G_MBIC_SERVICE" ||
        c.base_id === "G_MBNC" ||
 ...
 */
function calculerEcarts(calculs, scenario_reference_id) {
    ...
    return calculs.map((sc) => {
        ...
        return {
            ...
            DELTA_COTISATIONS: (inter.COTISATIONS_SOCIALES_NETTES ?? 0) -
                (ref.COTISATIONS_SOCIALES_NETTES ?? 0),
            DELTA_FISCAL: (inter.IR_ATTRIBUABLE_SCENARIO ?? 0) +
                (inter.IS_DU_SCENARIO ?? 0) -
                ((ref.IR_ATTRIBUABLE_SCENARIO ?? 0) + (ref.IS_DU_SCENARIO ?? 0)),
            DELTA_COUT_TOTAL: (inter.COUT_TOTAL_SOCIAL_FISCAL ?? 0) -
 ...
 */
function calculerScores(calcul, _params) {
    ...
    const aides_val = (inter.REDUCTION_ACRE ?? 0) + (inter.AIDE_ARCE_TRESORERIE ?? 0);
    const DEPENDANCE_AIDES_RATIO = net > 0 ? aides_val / Math.abs(net) : 0;
    // Score robustesse : plus le taux de prélèvement est élevé, moins robuste
    const TAUX_PRELEVEMENTS_GLOBAL = ca > 0 ? (inter.COUT_TOTAL_SOCIAL_FISCAL ?? 0) / ca : 0;
    ...
    const SCORE_GLOBAL_SCENARIO = w_net * net_normalise -
 ...
 */
function construireComparaison(calculs, scenario_reference_id, logger) {
 ...
 */
function determinerRecommandation(calculs, comparaison, logger) {
    if (calculs.length === 0)
        return null;
    // Scénario optimal selon score global
    const optimal = calculs.reduce((best, cur) => cur.scores.SCORE_GLOBAL_SCENARIO > best.scores.SCORE_GLOBAL_SCENARIO ? cur : best);
    const reference = calculs.find((c) => c.scenario_id === comparaison.scenario_reference_id);
    const ecart_ref = comparaison.ecarts.find((e) => e.scenario_id === optimal.scenario_id);
    const gain_annuel = ecart_ref?.DELTA_NET_APRES_IR ?? 0;
    const points_de_vigilance = [];
    if (optimal.scores.DEPENDANCE_AIDES_RATIO > 0.3) {
        points_de_vigilance.push(`Ce scénario dépend à ${Math.round(optimal.scores.DEPENDANCE_AIDES_RATIO * 100)} % ` +
    ...
    if (optimal.scores.SCORE_COMPLEXITE_ADMIN >= 4) {
        points_de_vigilance.push("Complexité administrative élevée (société IS). Coûts de gestion comptable à prévoir.");
    }
    if (optimal.niveau_fiabilite !== "complet") {
        points_de_vigilance.push(`Résultat de fiabilité "${optimal.niveau_fiabilite}" — certaines données sont estimées. ` +
...
// Helpers privés
// ─────────────────────────────────────────────────────────────────────────────
function _getScoreComplexiteBase(baseId) {
    const complexites = {
...
function _buildMotifRecommandation(optimal, comparaison, gain_annuel) {
    const parts = [];
