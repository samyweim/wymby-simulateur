...
exports.calculerSociete = calculerSociete;
const cotisations_tns_js_1 = require("./cotisations-tns.js");
const cotisations_assimile_js_1 = require("./cotisations-assimile.js");
const ir_js_1 = require("./ir.js");
const is_js_1 = require("./is.js");
/**
 * Calcule un scénario EURL ou SASU (IS ou IR).
 */
function calculerSociete(input, params, logger) {
    ...
    if (isISMode) {
        ...
        const resultat_apres_is = Math.max(0, RESULTAT_FISCAL_APRES_EXONERATIONS - IS_DU_SCENARIO);
        DIVIDENDES_DISTRIBUABLES = (0, is_js_1.f_dividendes_distribuables)(resultat_apres_is);
        // Dividendes effectivement envisagés (plafonnés au disponible)
        const dividendes_effectifs = Math.min(input.dividendes_envisages ?? DIVIDENDES_DISTRIBUABLES, DIVIDENDES_DISTRIBUABLES);
        if (input.type_societe === "SASU_IS") {
            // Président SASU : dividendes soumis PS uniquement (18,6 %)
            DIVIDENDES_NETS_PERCUS = (0, is_js_1.f_dividendes_nets_assimile)(dividendes_effectifs, params);
        }
        else {
            ...
            avertissements.push("EURL IS : assiette cotisations TNS sur dividendes (franchise 10 % capital) — " +
    ...
    if (input.type_societe === "SASU_IS" || input.type_societe === "SASU_IR") {
        // Assimilé-salarié
        const result = (0, cotisations_assimile_js_1.f_cotisations_assimile_salarie)(REMUNERATION_DEDUCTIBLE, params);
        ...
        const asuResult = (0, cotisations_tns_js_1.f_assiette_sociale_ASU)(assiette_base, params, logger, input.scenario_id);
    ...
    if (input.acre_active) {
        if (input.type_societe === "SASU_IS" || input.type_societe === "SASU_IR") {
            ...
            const taux_max = !input.date_creation || input.date_creation < dateCharniere
    ...
    const COTISATIONS_SOCIALES_NETTES = Math.max(0, COTISATIONS_SOCIALES_BRUTES - REDUCTION_ACRE);
    // ── Indicateurs finaux ────────────────────────────────────────────────────
...
//# sourceMappingURL=societes.js.map