...
function calculerMicro(input, params, logger) {
    ...
    const ABATTEMENT_FORFAITAIRE = Math.max(CA_HT_RETENU * taux_abattement, min_abattement);
    ...
    if (input.acre_active) {
        ...
        const taux_acre = !input.date_creation || input.date_creation < dateCharniere
    ...
    const COUT_TOTAL_SOCIAL_FISCAL = COTISATIONS_SOCIALES_NETTES + IR_ATTRIBUABLE_SCENARIO + input.TVA_NETTE_DUE;
...
// Helpers
// ─────────────────────────────────────────────────────────────────────────────
function _getTauxAbattement(type, params) {
    switch (type) {
...
function _getTauxSocialMicro(type, params) {
    switch (type) {
...
function _getTauxVFL(type, params) {
    switch (type) {
