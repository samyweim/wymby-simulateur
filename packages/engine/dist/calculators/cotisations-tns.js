...
exports.f_assiette_sociale_ASU = f_assiette_sociale_ASU;
exports.f_cotisations_tns_bic = f_cotisations_tns_bic;
exports.f_acre_hors_micro = f_acre_hors_micro;
/**
 * Calcule l'assiette sociale après application de l'ASU 2026.
 * Assiette = max(plancher, min(revenu_net × 0.74, plafond))
 */
function f_assiette_sociale_ASU(revenu_professionnel_net, params, logger, scenario_id) {
    ...
    if (revenu_professionnel_net <= 0) {
        return {
            assiette: plancher,
            hypothese: "Résultat nul ou négatif : assiette sociale = plancher ASU. " +
 ...
 */
function f_cotisations_tns_bic(assiette, params, logger, scenario_id) {
    const tnsParams = params.social.CFG_TAUX_SOCIAL_TNS_BIC;
    const pass = params.referentiels.CFG_PASS_2026;
    const detail = {};
    let cotisations_brutes = 0;
    // ── Maladie-maternité (tranches progressives) ──────────────────────────
    const maladie = _calculerCotisationsTranches(assiette, tnsParams.maladie_maternite.tranches, pass);
    ...
    const rc = _calculerCotisationsTranches(assiette, tnsParams.retraite_complementaire.tranches, pass);
    ...
    const af = _calculerCotisationsTranches(assiette, tnsParams.allocations_familiales.tranches, pass);
    ...
    if (cotisations_minimales_appliquees) {
        return {
            ...
            avertissement_minimales: `Résultat faible : cotisations calculées (${cotisations_brutes.toFixed(0)} €) inférieures ` +
 ...
 */
function f_acre_hors_micro(cotisations_brutes, assiette, params, date_creation) {
    ...
    const taux_max = !date_creation || date_creation < dateCharniere
...
// Helpers privés
// ─────────────────────────────────────────────────────────────────────────────
function _calculerCotisationsTranches(assiette, tranches, pass) {
    let cotisation = 0;
    for (const tranche of tranches) {
        ...
        if (assiette <= borne_inf)
            break;
        const assiette_tranche = Math.min(assiette, borne_sup) - borne_inf;
        cotisation += assiette_tranche * tranche.taux;
    }
    return cotisation;
}
//# sourceMappingURL=cotisations-tns.js.map