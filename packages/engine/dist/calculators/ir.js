...
exports.f_bareme_progressif = f_bareme_progressif;
exports.f_ir_attribuable_scenario = f_ir_attribuable_scenario;
/**
 ...
 */
function f_bareme_progressif(base_imposable, nb_parts, params) {
    if (base_imposable <= 0)
        return 0;
    ...
    const nb_parts_reference = qfParams.parts.couple_marie_pacse !== nb_parts
        ? qfParams.parts.celibataire
        : qfParams.parts.couple_marie_pacse;
    const impot_reference = _calculerImpotBrut(base_imposable, nb_parts_reference, tranches);
    ...
    const impot_plafonne = avantage_reel > avantage_max
 ...
 */
function f_ir_attribuable_scenario(base_ir_scenario, autres_revenus_foyer, autres_charges_foyer, nb_parts, params, logger, scenario_id) {
    const base_hors_scenario = Math.max(0, (autres_revenus_foyer ?? 0) - (autres_charges_foyer ?? 0));
    ...
    const mode = autres_revenus_foyer === undefined ? "estimation" : "complet";
...
// Helpers privés
// ─────────────────────────────────────────────────────────────────────────────
function _calculerImpotBrut(base_imposable, nb_parts, tranches) {
    if (base_imposable <= 0 || nb_parts <= 0)
        return 0;
...
function _appliquerDecote(impot_brut, nb_parts, params) {
    ...
    if (impot_brut > seuil)
        return impot_brut;
    const decote = Math.max(0, forfait - decoteParams.taux * impot_brut);
    return Math.max(0, impot_brut - decote);
}
function _getTrancheMarginaleApplicable(baseParPart, params) {
    const tranches = params.fiscal.CFG_BAREME_IR_TRANCHES;
    let taux = 0;
    for (const tranche of tranches) {
